...
from backend.logger.strategy_logger import log_strategy, get_user_strategy_history  # 📊 Logging module
import os
import json
...
def process_belief(request: BeliefRequest) -> Dict[str, Any]:
    try:
        result = run_ai_engine(request.belief)  # 🧠 Run core AI engine
        log_strategy(request.belief, result["strategy"])  # 📊 Log to history
        return result
...
# === GET /strategy_history ===
# → Returns all saved strategies for a given user (default = anonymous)
